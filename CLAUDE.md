...
- **現金の金種**（`EventData.cashBreak`）：設定の予算カードと当日タブの「金種」ボタン（`A.wallet`）から枚数を入れる。
  ...
  テンキーの `opt.hint` に出す（現金のときだけ）。計算は金種ごとの個数制限つきDP（`planFor`）で、手持ちを超える案は出ない。
  枚数を入れている間は `cash`（財布の現金）は自動計算になる（入力欄は readonly）。
  **現金で払ったら財布の中身から自動で引く**（1.3.2〜。本人に「引く」操作はさせない。多めに出しておつりをもらっても合計は代金ぶんだけ減るため）。
  テンキーで記録したときは `deductCash`、チェックだけで完了したときは `afterFinish` の `settleCash`（`S.cashUnpaid(cid)`＝
  現金で買った額 − すでに引いた額）が `S.applyPay(plan, cid, { noUndo: true })` を呼び、出した分を引いて**おつりを金種に戻す**。
  `entry.cashPaid` に控えるので二重には引かれない。`noUndo` なので取り消しの控えは増えず、「元に戻す」で購入の記録と財布がまとめて戻る。
  手持ちの金種で払えない（足りない）ときは金種は変えず、`stats().cashLeft` がマイナスで出る。
...
- リリースのたびに **`sw.js` の `CACHE`** と **`js/app.js` の `HC.VERSION`** を同じ番号に上げる（現在 1.3.2）
...
- [x] 操作性の見直し（1.3.1）：テンキーの確定ボタンを常に表示（iPhone SE で下が切れていた）、地図下の区間の帯がタブバーの高さぶん浮いていたのを修正、ルート線を非表示にしていても区間表示ではその区間を描く、区間を画面の上寄りに表示、当日カードは品物を画像より先に、固定ボタンを少し低く、当日モードで上部の「残り」を省略、シートを開いている間はトーストを画面上部に、小さい当たり判定と文字の修正
- 確認の基準：360px・375×667（iPhone SE）・390×844・PC で、当たり判定36px以上・横はみ出しなし・本文11.5px以上・全ボタンの自動押下でエラー0
- [x] 現金で払ったら財布の中身から自動で引く（完了トーストの「財布から ¥N 引く」とテンキーの「財布の中身から引く」を廃止。元に戻すで一緒に戻る）（1.3.2）
